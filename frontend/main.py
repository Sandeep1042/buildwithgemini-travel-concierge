...
def _get_env_or_file(env_key, filename, json_key, default=None):
    ...
    if os.path.exists(path):
        try:
            if filename.endswith(".json"):
                import json
                with open(path) as f:
                    val = json.load(f).get(json_key)
            else:
                import yaml
                with open(path) as f:
                    val = yaml.safe_load(f).get(json_key)
            if val:
                return val
        except Exception:
            pass
    if default is not None:
        return default
    raise KeyError(f"{env_key} environment variable not set.")

RESOURCE = _get_env_or_file("AGENT_ENGINE_RESOURCE_NAME", "deployment_metadata.json", "remote_agent_runtime_id")
# The agent's app directory (matches agent_directory in agents-cli-manifest.yaml).
AGENT_DIRECTORY = _get_env_or_file("AGENT_DIRECTORY", "agents-cli-manifest.yaml", "agent_directory", default="app")
